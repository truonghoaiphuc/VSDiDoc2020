...
namespace Lotus.Base
{
    public partial class FrmBase : XtraForm
    {
        ...
        #region Validate
        protected ConditionValidationRule notEmptyValidationRule;

        //Khởi tạo ValidationRule
        protected void InitValidationRule()
        {
            notEmptyValidationRule = new ConditionValidationRule();
            notEmptyValidationRule.ConditionOperator = ConditionOperator.IsNotBlank;
            notEmptyValidationRule.ErrorText = "Trường này không được bỏ trống";
            notEmptyValidationRule.ErrorType = ErrorType.Default;
        }

        //Hàm thiết lập Validate giữa 2 control
        protected void SetCompareValidationRule(Control control, Control refControl, string ErrText)
        {
            CompareAgainstControlValidationRule comValidationRule = new CompareAgainstControlValidationRule();
            comValidationRule.Control = refControl;
            comValidationRule.CompareControlOperator = CompareControlOperator.GreaterOrEqual;
            comValidationRule.ErrorText = ErrText;
            comValidationRule.CaseSensitive = false;
            DXValidationProvider dxValid = new DXValidationProvider();
            dxValid.SetValidationRule(control, comValidationRule);
        }
        protected void SetValidationRule(params Control[] control)
        {
            foreach (var i in control)
                dxValid.SetValidationRule(i, notEmptyValidationRule);
        }
        #endregion
    }
}